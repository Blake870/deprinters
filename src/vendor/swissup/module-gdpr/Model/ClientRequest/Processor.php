...
class Processor
{
     ...
     * @param \Swissup\Gdpr\Model\ResourceModel\PersonalDataHandler\CollectionFactory $handlers
    ...
    private function export(ClientRequest $request)
    {
        ...
        foreach ($this->handlers as $handler) {
            try {
                $data = array_replace_recursive($data, $handler->export($request));
            } catch (\Exception $e) {
