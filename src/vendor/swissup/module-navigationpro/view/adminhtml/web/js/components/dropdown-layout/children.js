...
], function ($, _, registry, mageTemplate, template, makeVerticalColumns) {
    'use strict';
    ...
    return function (item, provider) {
        ...
        function renderTree(currentItem, currentRelativeLevel) {
            ...
            _.find(children, function (el, i) {
                if (getMaxChildrenCount() <= i && currentRelativeLevel === 1) {
                    html += '<div><span class="navpro-all"><strong>Shop All</strong></span></div>';
        ...
        return {
            ...
            afterRender: function (container) {
                var el = $('.navpro-content-cols', container);

                if (item.direction === 'vertical') {
                    makeVerticalColumns(el, item.columns_count);
                } else {
                    el.css({
                        height: 'auto'
                    });
