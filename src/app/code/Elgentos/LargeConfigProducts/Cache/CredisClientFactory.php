...
class CredisClientFactory
{
    ...
    public function create(): \Credis_Client
    {
        ...
        if (isset($cacheSetting['frontend']['elgentos_largeconfigproducts']['backend_options'])) {
            ...
            $password = $this->scopeConfig->getValue('elgentos_largeconfigproducts/prewarm/redis_password') ?? null;
            if (empty($password)) {
                $password = null;
            }
        }

        return new \Credis_Client($server, $port, $timeout, $persistent, $database, $password);
    }
}
